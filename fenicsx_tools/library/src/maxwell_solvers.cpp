...
namespace mag_tools{
    void magnetic_field_2D_Az(int argc, char* argv[]){
        ...
        // read input file and create description
        ...
        const auto nuDiff = std::make_shared<dolfinxFunction<T>>(quadMatFS); // differential reluctivity  
        const auto M  = std::make_shared<dolfinxFunction<T>>(quadVecFS); // variable magnetization
        const auto M0 = std::make_shared<dolfinxFunction<T>>(quadVecFS); // constant magnetiztaion    
        const auto j  = std::make_shared<dolfinxFunction<T>>(quadScaFS); // source current density
        ...
        // output of B,H and M -> projection to dg-functionspace
        ...
        // output of source current density
        auto j_out = std::make_shared<dolfinxFunction<T>>(dgScaFS);

        // setup appropriate projections
        auto dgProjVec = mag_tools::dg_projection<T>(mesh, M->function_space()->mesh()->topology()->dim());
        ...
        // coupling of global jacobian / differential reluctivity
        ...
        // setup energy calculation -> currently experimental, not correct for most nonlinear materials
        ...
        if (scen.forceCalc.size() > 0){
            auto _Fcalc = std::make_shared<force_calculation<T>>(force_calculation<T>(A, Bquad, Hquad, nuDiff, wmag, meshContainer));
            forceCalculation.push_back(_Fcalc);
            _Fcalc->set_domain_depth(scen::as_double(scen.forceCalc[0], "domainDepth"));
            _Fcalc->set_mag_domains(magDom);
            for(auto& FcalcDef: scen.forceCalc){
                _Fcalc->add_force_calc(FcalcDef);
            }
        }

        // debug functionality: check if body is surrounded by air
        ...
        // setup field queries -> TODO: whole step should be moved to proper post processing
        ...
        while (!timeStepping.is_finished()){
            ...
            // output
            B = dgProjVec.project_quad_to_dg(Bquad, B);
            H = dgProjVec.project_quad_to_dg(Hquad, H);
            M_out = dgProjVec.project_quad_to_dg(M, M_out);
            M0_out = dgProjVec.project_quad_to_dg(M0, M0_out);
            ...
            wmag->perform_update();
            for (auto& _fCalc:forceCalculation){
                _fCalc->update_forces();
                _fCalc->store_results(timeStepping.get_time());
            }
        ...
        for (auto& _fCalc:forceCalculation){
           _fCalc->append_forces_to_output(xmlOutput);
        }
